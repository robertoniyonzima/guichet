...
<body>
    ...
    if (isset($_POST['action'])) {
        ...
        switch ($action) {
            case 'activer':
                $sql = "UPDATE useraute SET status = 1 WHERE id = :id";
            ...
            case 'desactiver':
                $sql = "UPDATE useraute SET status = 0 WHERE id = :id";
                $stmt = $connect->prepare($sql);
                $stmt->bindParam(':id', $userId, PDO::PARAM_INT);
                $stmt->execute();
            ...
            case 'supprimer':
                $sql = "DELETE FROM useraute WHERE id = :id";
                $stmt = $connect->prepare($sql);
                $stmt->bindParam(':id', $userId, PDO::PARAM_INT);
                $stmt->execute();
    ...
    $sql = "SELECT id, username, status FROM useraute";
    $stmt = $connect->prepare($sql);
    $stmt->execute();
